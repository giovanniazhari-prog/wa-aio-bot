...
────────────────────────────────────`
    ...
    if (hasUrl && !lower.startsWith(".")) {
        ...
        // Kirim pesan tunggu → simpan key → hapus lalu kirim video
          const waitMsg = await reply(`⏳ Bentar ya, lagi download dari ${platform.emoji} *${platform.name}*...`);
          await react("⏳");
          console.log('[DELETE-DEBUG] waitMsg.key =', JSON.stringify(waitMsg?.key));
        ...
        try {
            ...
            if (waitMsg?.key) {
                ...
                console.log('[DELETE-DEBUG] delete result:', JSON.stringify(delResult?.key));
            ...
            await react("❌");
            if (waitMsg?.key) sock.sendMessage(jid, { delete: waitMsg.key }).catch(() => {});
